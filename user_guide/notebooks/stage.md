...
def create_stage_directories(manifest: dict, console: Console, gitignore=True):
    ...
    ----------
    manifest : dict
        Manifest containing the ``project`` directory configuration.
    console : Console
        Rich console used for progress messages.
    gitignore : bool, default=True
        Whether to create a minimal ``.gitignore`` file in each directory.

    Raises
    ------
    ValueError
        Raised when one or more required project directories are missing
        from the manifest.
    """
...
Let’s create a quick `StageResult` class to represent the results of
staging operations, so we can report them nicely in the UI and
potentially use them in the future for more detailed reporting:
...
Next, we can work on fetching the data from Globus. Fortunately, much of
the work has been done for us in the `customs` module, so we just need
...
def stage_data_from_globus(
    ...
    console : Console
        Rich console used for progress messages.
    issue_transfer : bool, default=True
    ...
    ------
    ValueError
        Raised when the manifest does not define the required Globus
        source information.
    """
...
def stage_data_from_fasrc(
    ...
    ----------
    manifest : dict
        Manifest containing the ``01_gold_mine`` source configuration.
    console : Console
        Rich console used for progress messages.
    ...
    -------
    StageResult
        Summary of symlink creation outcomes for all requested items.
    """
    ...
    clearance = check_gold_mine_clearance(fasrc_info)
    
    if not clearance.cleared:
        return StageResult(
            source="01_gold_mine",
            staged=False,
            message=f"Gold Mine clearance failed: {clearance.message}",
            details=task_list
        )
...
That should do it for now! We will implement the dataverse staging later
on.
